...
export function createNeonPool(dsn: string): pg.Pool {
...
export async function neonSelectByUser(
  pool: pg.Pool,
  userId: string,
  limit: number
): Promise<MixedData[]> {
...
export async function neonSelectByType(
  pool: pg.Pool,
  type: string,
...
export async function neonSelectById(pool: pg.Pool, id: string): Promise<MixedData | undefined> {
  const result = await pool.query<MixedData>(
    `SELECT id, user_id, type, subtype, score_value, payload, file_url, created_at, updated_at
     FROM mixed_data
     WHERE id = $1
...
export async function neonDeleteById(pool: pg.Pool, id: string): Promise<void> {
  await pool.query(`DELETE FROM mixed_data WHERE id = $1`, [id]);
}

export async function neonCountByType(pool: pg.Pool, type: string): Promise<number> {
...
export async function neonGetMetaUsedBytes(pool: pg.Pool, dbName: string): Promise<number> {
...
export async function neonUpdateMetaUsedBytes(
  pool: pg.Pool,
  dbName: string,
  maxBytes: number,
  usedBytes: number
